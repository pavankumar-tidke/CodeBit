...
export const getServerSideProps = checkLogin(async (ctx, loginRes) => {
  ...
  if (loginRes == "true") {
    ...
    const res = await axios.get(`http://localhost:5000/login_discover`, {
    ...
    const res = await axios.get(`http://localhost:5000/discover`);
    const { data } = res;
    if (!data) {
      return {
        notFound: true,
...
const Index = (props) => {
  ...
  return (
    <>
      ...
      <div className={`min-h-screen mt-14 ${sty.card}`}>
